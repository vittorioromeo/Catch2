...
namespace Catch {
    namespace Detail {
        ...
        struct TupleElementPrinter {
            static void print( const Tuple& tuple, ReusableStringStream& rss ) {
                rss << ( N ? ", " : " " )
                   << ::Catch::Detail::stringify( std::get<N>( tuple ) );
                TupleElementPrinter<Tuple, N + 1>::print( tuple, rss );
            }
        };

        template <typename Tuple, std::size_t N>
        struct TupleElementPrinter<Tuple, N, false> {
            static void print( const Tuple&, ReusableStringStream& ) {}
        };
