    ...
    namespace Benchmark {
        struct Benchmark {
            ...
            ExecutionPlan prepare(const IConfig &cfg, Environment env) const {
                ...
                auto run_time = std::max(min_time, std::chrono::duration_cast<decltype(min_time)>(std::chrono::milliseconds(cfg.benchmarkWarmupTime())));
                auto&& test = Detail::run_for_at_least<Clock>(std::chrono::duration_cast<IDuration>(run_time), 1, fun);
                int new_iters = static_cast<int>(std::ceil(min_time * test.iterations / test.elapsed));
                return { new_iters, test.elapsed / test.iterations * new_iters * cfg.benchmarkSamples(), fun, std::chrono::duration_cast<FDuration>(std::chrono::milliseconds(cfg.benchmarkWarmupTime())), Detail::warmup_iterations };
            }
